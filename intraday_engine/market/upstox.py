...
class UpstoxREST:
    ...
    def _get(self, path: str, params: dict | None = None) -> dict:
        response = self.session.get(BASE + path, headers=self.headers, params=params, timeout=self.timeout)
        response.raise_for_status()
        return response.json()

    def search_instruments(self, query: str, exchanges: str = "NSE", segments: str = "EQ", records: int = 30, page_number: int = 1) -> list[dict]:
        payload = self._get(
            "/v2/instruments/search",
            {"query": query, "exchanges": exchanges, "segments": segments,
             "page_number": page_number, "records": min(records, 30)},
        )
    ...
    def intraday_candles(self, instrument_key: str, unit: str = "minutes", interval: int = 1) -> pd.DataFrame:
        key = quote(instrument_key, safe="")
        payload = self._get(f"/v3/historical-candle/intraday/{key}/{unit}/{interval}")
        return self._frame(payload.get("data", {}).get("candles", []))

    def historical_candles(self, instrument_key: str, unit: str, interval: int, to_date: date, from_date: date | None = None) -> pd.DataFrame:
        key = quote(instrument_key, safe="")
    ...
    def full_market_quotes(self, instrument_keys: list[str]) -> dict:
        if not instrument_keys:
            return {}
        payload = self._get("/v2/market-quote/quotes", {"instrument_key": ",".join(instrument_keys)})
        return payload.get("data", {})
