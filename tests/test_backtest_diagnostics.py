...
def test_trade_diagnostics_preserves_signal_metadata_and_excursions():
    signal = TradeSignal(
        action="BUY",
        score=72.0,
        confidence=72.0,
    ...
    assert diagnostics.loc[0, "mae_points"] == 1.0
    assert diagnostics.loc[0, "mfe_points"] == 4.0
    assert diagnostics.loc[0, "mae_r"] == 0.5
    assert diagnostics.loc[0, "mfe_r"] == 2.0
    assert "price is above VWAP" in diagnostics.loc[0, "reasons"]


def test_summarize_diagnostics_separates_score_magnitude_by_side():
    trades = pd.DataFrame(
        [
            {
                "symbol": "TEST",
    ...
    assert by_side["trades"].sum() == 2
    assert by_score["trades"].sum() == 2
    assert set(by_score["side"]) == {"LONG", "SHORT"}
    assert not by_reason.empty
    assert by_excursion["trades"].sum() == 2
    assert by_excursion.loc[by_excursion["side"] == "LONG", "avg_mfe_r"].iloc[0] == 2.0
