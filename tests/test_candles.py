...
def test_date_windows_cover_range_without_overlap():
    assert list(date_windows(date(2026, 1, 1), date(2026, 2, 10), max_days=30)) == [
...
def test_normalize_candles_sorts_and_deduplicates():
    ...
    normalized = normalize_candles(raw, instrument_key="NSE_EQ|TEST", symbol="test", interval="1m")
...
def test_quality_report_detects_duplicate_and_bad_ohlc():
    frame = pd.DataFrame({
        ...
        "open": [100, 100], "high": [99, 101], "low": [98, 99],
        "close": [98.5, 100], "volume": [100, -1],
    })
